...
public class PresidentData {
    ...
    public static ArrayList<PresidentSetGet>getListdata(){
        PresidentSetGet presidentSetGet = null;
        ArrayList<PresidentSetGet> list = new ArrayList<>();

        for (String[] aData : data){
            presidentSetGet = new PresidentSetGet();
            presidentSetGet.setName(aData[0]);
            presidentSetGet.setRemarks(aData[1]);
            presidentSetGet.setPhoto(aData[2]);
            list.add(presidentSetGet);
        }
        return list;
    }
